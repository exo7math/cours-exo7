...
\bigskip 
Sur la figure de gauche, la dérivée partielle  $\frac{\partial f}{\partial x}$ indique la pente en un point d'une tranche parallèle à l'axe $(Ox)$\couleurnb{ (en orange)}{}. Sur la figure de droite, la dérivée partielle  $\frac{\partial f}{\partial y}$ indique la pente en un point d'une tranche parallèle à l'axe $(Oy)$\couleurnb{ (en vert)}{}.
...
Ci-dessous, la dérivée directionnelle $D_vf$ indique la pente en un point d'une tranche \couleurnb{(en rouge)}{} dans la direction d'un vecteur $v$.
...
L'application $\ell $ est la \defi{différentielle} de $f$ en $x_0$ et se note $\dd f(x_0)$.
\end{definition}
...
\begin{definition}
Soit $f: \Rr^n \to \Rr$. On dit que $f$ est de \defi{classe $\mathcal{C}^1$} si les dérivées partielles $\frac{\partial f}{\partial x_i}$
...
Une autre façon de dire que $f$ est différentiable est de dire que $f$ admet un \defi{développement limité à l'ordre~$1$}. Pour $f : \Rr^2 \to \Rr$, si $f$ est différentiable au point $(x_0,y_0)$, alors
